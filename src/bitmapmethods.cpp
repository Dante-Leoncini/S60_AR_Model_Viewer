...
CFbsBitmap* NBitmapMethods::CreateBitmapLC(TSize aSizeInPixels,TDisplayMode aDispMode)
	...
	ASSERT((bitmap->DisplayMode() == KMColorDepth) || (bitmap->DisplayMode()  == EGray2));
	return bitmap;
	}
...
CFbsBitmap* NBitmapMethods::CreateBitmapLC(const TDesC& aFileName,TInt aId)
	...
	if ((bitmap->DisplayMode() == KMColorDepth) || (bitmap->DisplayMode()  == EGray2) )
		{
			return bitmap;
		}
	else
		{
		CFbsBitmap* newBitmap = CreateBitmapLC(	bitmap->SizeInPixels(),KMColorDepth);															
